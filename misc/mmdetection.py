to replace demo/image_demo.py in v3.2.0 of mmdetection githiub source code
...
def main():
    ...
    inputPath = os.path.abspath(call_args['inputs'])
    ...
    ret = inferencer(**call_args)
    
    classes = get_classes("coco")
    # print(" ... classes", len(classes), classes)
    labels = ret['predictions'][0]['labels']
    scores = ret['predictions'][0]['scores']
    bboxes = ret['predictions'][0]['bboxes']
    outputJson = {}
    outputJson['input'] = inputPath
    outputJson['predictions'] = []
    for index in range(len(labels)):
        if scores[index] > 0.3:
            outputJson['predictions'].append(
                {
                    'label': labels[index],
                    'class': classes[labels[index]],
                    'score': scores[index],
                    'bbox': bboxes[index]
                }
            )
            print(" ... ", labels[index], scores[index], classes[labels[index]], bboxes[index])
    # print(" ... labels", len(labels), labels)
    
    with open(outputPath, 'w') as fout:
        json_dumps_str = json.dumps(outputJson, indent=4)
